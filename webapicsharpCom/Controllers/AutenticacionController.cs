...
namespace webapicsharp.Controllers
{
    ...
    public class AutenticacionController : ControllerBase
    {
            ...
            IServicioCrud servicioCrud)
        {
            ...
            ValidarConfiguracionJwt(); //problemas al generar token
        }
        private void ValidarConfiguracionJwt()
        {
            if (string.IsNullOrWhiteSpace(_configuracionJwt.Key))
            {
                throw new ArgumentException("La clave JWT no está configurada en appsettings.json");
            }

            if (_configuracionJwt.Key.Length < 16)
            {
                throw new ArgumentException($"La clave JWT es demasiado corta. Longitud mínima: 16 caracteres. Longitud actual: {_configuracionJwt.Key.Length}");
            }

            if (string.IsNullOrWhiteSpace(_configuracionJwt.Issuer))
            {
                throw new ArgumentException("El Issuer JWT no está configurado");
            }

            if (string.IsNullOrWhiteSpace(_configuracionJwt.Audience))
            {
                throw new ArgumentException("El Audience JWT no está configurado");
            }
        }
